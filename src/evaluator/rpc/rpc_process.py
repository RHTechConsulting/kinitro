...
class RPCRequest:
    ...
    @classmethod
    def create_ping(cls, message: str, timeout: float = 5.0) -> 'RPCRequest':
    ...
    @classmethod
    def create_act(cls, obs: np.ndarray, timeout: float = 5.0) -> 'RPCRequest':
    ...
    @classmethod
    def create_reset(cls, timeout: float = 5.0) -> 'RPCRequest':
...
class RPCResponse:
    ...
    def success(self) -> bool:
        ...
        return (self.processing_error is None and 
                self.server_response is not None and 
                self.server_response.success)
    ...
    @classmethod
    def from_server_response(cls, request_id: str, server_response: ServerResponse) -> 'RPCResponse':
...
class RPCProcess:
    ...
    async def handle_request(self, request: RPCRequest) -> RPCResponse:
        try:
            ...
            if request.method == RPCMethod.PING:
            ...
            elif request.method == RPCMethod.ACT:
            ...
            elif request.method == RPCMethod.RESET:
                ...
                server_response = ServerResponse.from_error(
                    request.method, 
                    f"Unknown RPC method: {request.method}"
                )
    ...
    async def process(self):
        ...
        try:
            ...
            if startup_response.success:
                print(f"RPC process for {self.host}:{self.port} connected successfully, server says: {startup_response.result}")
            else:
                print(f"RPC process for {self.host}:{self.port} connection test failed: {startup_response.error_message}")
                return
            ...
            while True:
                try:
                    ...
                    print(f"RPC[{self.host}:{self.port}] received: method={request.method.value}, id={request.request_id[:8]}")
